...
final class Version20260615173812 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__site_checks AS SELECT id, type, config, is_active, check_interval_minutes, run_at_time, retention_days, client_id FROM site_checks');
        $this->addSql('DROP TABLE site_checks');
        $this->addSql('CREATE TABLE site_checks (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, type VARCHAR(64) NOT NULL, config CLOB NOT NULL, is_active BOOLEAN NOT NULL, check_interval_minutes INTEGER NOT NULL, run_at_time VARCHAR(5) DEFAULT NULL, retention_days INTEGER DEFAULT NULL, client_id INTEGER NOT NULL, CONSTRAINT FK_EF22AE2619EB6921 FOREIGN KEY (client_id) REFERENCES clients (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO site_checks (id, type, config, is_active, check_interval_minutes, run_at_time, retention_days, client_id) SELECT id, type, config, is_active, check_interval_minutes, run_at_time, retention_days, client_id FROM __temp__site_checks');
        $this->addSql('DROP TABLE __temp__site_checks');
        $this->addSql('CREATE INDEX IDX_EF22AE2619EB6921 ON site_checks (client_id)');
    }
}
